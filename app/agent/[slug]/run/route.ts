// app/api/agents/[slug]/run/route.ts
...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    // فقط اگر Agent برای کاربر فعال باشد
    ...
    if (!userAgent) {
      return NextResponse.json(
        { error: "ابتدا این Agent را فعال کنید" },
        { status: 403 }
      );
    }
    ...
    try {
      const output = await runAgent(slug, input);
      ...
      return NextResponse.json(
        { error: err?.message || "اجرای Agent ناموفق بود" },
        { status: 500 }
      );
    }
  } catch (err) {
